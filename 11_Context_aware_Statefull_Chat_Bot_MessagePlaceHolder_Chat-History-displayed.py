...
while True:
    ...
    try:
        ...
        #=============================
        # Display Current Chat History Content
        #=============================
        print("\n--- [CURRENT CHAT HISTORY STATUS] ---")
        if not chat_history:
            print("Chat history is empty.")
        else:
            # We step by 2 because 1 complete interaction = 1 HumanMessage + 1 AIMessage
            for index, i in enumerate(range(0, len(chat_history), 2), start=1):
                user_msg = chat_history[i]
                ai_msg = chat_history[i+1] if (i+1) < len(chat_history) else None
                
                print(f"\n🔄 Interaction Turn #{index}:")
                print(f" 👤 Appended: {type(user_msg).__name__} -> content='{user_msg.content}'")
                if ai_msg:
                    print(f" 🤖 Appended: {type(ai_msg).__name__} -> content='{ai_msg.content}'")
        print("--------------------------------------")
